...
export const useMentionAutocomplete = ({ mentionables, onApply }: UseMentionAutocompleteArgs) => {
    ...
    const items = useMemo(() => {
        ...
        const matched = query ? mentionables.filter(m => m.name.toLowerCase().includes(query)) : mentionables;
...
export const MentionAutocomplete = ({ items, activeIndex, onSelect }: MentionAutocompleteProps) => (
    <div
        role="listbox"
        className="absolute bottom-full left-0 z-20 mb-2 w-64 overflow-hidden rounded-lg border border-hairline bg-elevated p-1 shadow-overlay"
    >
        {items.map((item, i) => (
            <button
                key={item.id}
