...
import { foldReactions } from './foldReactions';
...
describe('foldReactions', () => {
    it('groups reactors under the emoji they used', () => {
        ...
        expect(tallies.get('C1:1')).toEqual([{ emoji: '👍', userIds: ['ada', 'bob'], mine: false }]);
    });
    ...
    it('keeps the newest event per person and emoji regardless of arrival order', () => {
        ...
        expect(foldReactions(outOfOrder, ME).get('C1:1')).toEqual([{ emoji: '👍', userIds: ['ada'], mine: false }]);
    });
