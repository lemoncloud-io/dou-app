...
const LandedAt = () => <span data-testid="landed">{useLocation().search}</span>;
...
const landOn = async (initialPath: string) => {
    ...
    return new URLSearchParams(screen.getByTestId('landed').textContent ?? '');
};
...
describe('ShareLinkRedirect — /s 공유 링크 리다이렉트', () => {
    it('주소 파라미터가 없는 code-only 릴레이 링크를 relay=1 초대 진입으로 바꿔 보낸다', async () => {
        ...
        const params = await landOn('/s?code=invt%3A1000072-2%3Ae3faf0d0');
    ...
    it('relay 플래그가 붙은 링크도 동일하게 처리한다', async () => {
        const params = await landOn('/s?code=c&relay');
    ...
    it('클라우드 폼은 _backend를 조립해 보낸다', async () => {
        const params = await landOn('/s?code=c&api=uzjpiaey7a&stage=dev');
    ...
    it('변환할 수 없는 링크는 파라미터 없이 루트로 보낸다', async () => {
        ...
        const params = await landOn('/s?nonsense=1');
