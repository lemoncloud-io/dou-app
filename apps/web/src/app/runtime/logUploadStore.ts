 ...
 *
 * This is a different key and a different store from the ring buffer's
 * `@chatic/web.log.queue` — that one is tab-scoped on purpose.
 *
...
/**
 * Drops every pending queue on this origin, not just this tab's.
 *
 * Used on logout: the device should not keep holding the ended session's logs,
 * and leaving other tabs' queues behind would let the next person's tab adopt
 * them as orphans and upload them under their own signed session.
 */
export const clearAllPendingQueues = (): void => {
    try {
        const doomed: string[] = [];
        for (let i = 0; i < localStorage.length; i += 1) {
            const candidate = localStorage.key(i);
            if (candidate?.startsWith(KEY_PREFIX)) doomed.push(candidate);
        }
