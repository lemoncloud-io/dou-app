...
import { APP_SYNC_EVENT_NAME, useChannelRepository } from '../../index';
 ...
 */
export const useChannels = (initialParams: ChatMinePayload) => {
    ...
    const currentParamsRef = useRef<ChatMinePayload>(initialParams);
    /**
     *  로컬 DB에서 채널 목록 읽어오기 및 정렬
     */
    const requestFromLocal = useCallback(
        async (params?: ChatMinePayload) => {
            try {
                setIsError(false);
                if (params) {
    ...
    return {
        ...
        isSyncing,
        isError,
        refresh: (options?: ChatMinePayload) => {
            void requestFromLocal(options);
            requestFromNetwork(options);
        },
        sync: (options?: ChatMinePayload) => requestFromNetwork(options),
    };
};
