 ...
 *
 * **The pointer boundary is the delicate part.** The entire bubble is a long-press target
 * (ChannelMessageRow listens for pointerdown/up/leave/cancel and contextmenu on the wrapping span),
 * so a press that starts on this button would also start the long-press timer and open the action
 * sheet on top of the copy. Links solve their version of this after the fact, by swallowing the
 * click that follows a fired long-press; a button can do better and stop the gesture from ever
 * starting, so every one of those events is stopped here at the button.
 *
...
export const MessageCodeBlock = ({ code, lang }: MessageCodeBlockProps) => {
    ...
    const handleCopy = () => {
        void (async () => {
            try {
                await copyMessageToClipboard(code);
                setCopied(true);
                if (timerRef.current !== null) window.clearTimeout(timerRef.current);
