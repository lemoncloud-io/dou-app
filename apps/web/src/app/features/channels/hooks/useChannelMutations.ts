...
export const useChannelMutations = () => {
    ...
    const createChannel = useCallback(
        (payload: ChannelCreateInput): Promise<DomainChannel> => run('start', () => channelRepository.createChannel(payload)),
        [channelRepository, run]
    );

    const updateChannel = useCallback(
        (payload: ChannelUpdateInput): Promise<DomainChannel> => run('update', () => channelRepository.updateChannel(payload)),
        [channelRepository, run]
    );

    const deleteChannel = useCallback(
        (payload: ChannelDeleteInput): Promise<DomainChannel> => run('delete', () => channelRepository.deleteChannel(payload)),
        [channelRepository, run]
    );

    const leaveChannel = useCallback(
        (payload: ChatLeaveInput): Promise<DomainChannel> => run('leave', () => channelRepository.leaveChannel(payload)),
        [channelRepository, run]
    );

    const inviteChannel = useCallback(
        (payload: ChatInviteInput): Promise<DomainChannel> => run('invite', () => channelRepository.inviteChannel(payload)),
        [channelRepository, run]
    );
