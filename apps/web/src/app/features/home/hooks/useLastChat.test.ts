...
const chat = (chatNo: number, content: string): DomainChat => ({ chatNo, content }) as unknown as DomainChat;
...
describe('useLastChat — 홈 행의 마지막 메시지', () => {
    it('limit 1로 chat 캐시를 관측한다', () => {
        seedChats([]);
        renderHook(() => useLastChat('ch-1'));
        expect(chatObserveList).toHaveBeenCalledWith({ channelId: 'ch-1', limit: 1 }, expect.any(Function));
    });
