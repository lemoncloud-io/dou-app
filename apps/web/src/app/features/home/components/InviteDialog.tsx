 ...
 * fully-formed invite entry (`provider=invite` + `code` + `_backend`). This lets the host page
 * (home) mount it unconditionally — it stays invisible on normal navigation and only surfaces
 * the popup when an invite deeplink lands. Dismiss/back strips the query string so the popup
 * cannot reappear on remount.
 */
export const InviteDialog = (): JSX.Element | null => {
    ...
    const params: InviteParams = useMemo(() => parseInviteDeeplink(location.search), [location.search]);
    const { data: inviteInfo } = useInviteInfo(params.code, params.backend);
    const { accept, isAccepting, missingDelegator, hasError } = useInviteAccept(inviteInfo);
    ...
    const targetName = inviteInfo?.name ?? inviteInfo?.site$?.name;
    ...
    // Show invite accept UI. The cloud/site descriptor is only known after login, so the prompt
    // stays generic until acceptance completes.
    return (
