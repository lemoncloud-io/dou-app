 ...
 *
 * The design has these as placeholders (a repeated icon and lorem copy), so the wording carried
 * over from the cloud guide this screen replaced rather than being invented here; the icons are
 * picked from the kit to match what each line actually describes.
 */
const BENEFITS = [
    { key: 'item1', Icon: IconGroup },
    { key: 'item2', Icon: IconChatAdd },
    { key: 'item3', Icon: IconUsersGroup },
] as const;
                    ...
                    <li key={key} className="flex flex-col gap-1">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-[16px] font-semibold leading-[1.4] tracking-[-0.015em] text-foreground">
                                {t(`mypage.subscription.benefits.${key}Title`)}
                            </span>
                        </div>
                        <p className="pl-11 text-[14px] leading-[1.5] tracking-[-0.015em] text-[#78828A]">
                            {t(`mypage.subscription.benefits.${key}Description`)}
                        </p>
                    </li>
                ))}
            </ul>
