...
1. **프로필** — 아바타 60px + 이름 + 이메일, `→ ROUTES.mypage.account.info`. 게스트는 "로그인하기" 행으로 대체
...
- **게스트/소셜로그인 변형 미확인**: 참조한 두 노드는 로그인 상태뿐이라, 게스트 루트는 기존 "로그인하기" 분기를 새 카드 레이아웃에 옮긴 것이다. 별도 변형 노드가 있으면 대조 필요.
