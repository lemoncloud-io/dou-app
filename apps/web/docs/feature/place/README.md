...
| `PlaceDetailPage`        | `/place/:placeId/settings/detail`   | 플레이스 정보 조회(이름·소개·만든 날짜·소유자) — 읽기 전용, 전원 접근 가능 |
| `PlaceEditPage`          | `/place/:placeId/settings/edit`     | 플레이스 이름(1~20자)·소개(≤100자)·프로필 이미지 편집 — 오너 전용          |
...
설계·데이터 흐름 상세는 [place-settings.md](place-settings.md), 소개 문구는 [place-desc.md](place-desc.md) 참조.
...
features/place/
  ...
  pages/PlaceEditPage.tsx            # 이름/소개/이미지 편집 (오너)
...
- **쓰기(플레이스 이름/소개/이미지)**: home의 `useUpdatePlace({ id, sid, name?, desc?, thumbnail? })` → `repos.place.updatePlace(...)`. 썸네일은 150px 정사각 리사이즈 후 base64. `desc`·`thumbnail`은 **바뀌었을 때만** 실어 부분 업데이트가 서로를 덮지 않게 한다 — 소개를 비운 저장은 `desc: ''`로 나가 서버에서 지워진다.
...
- **소개 문구는 relay 예외를 타지 않는다**(ADR-0074): 만든 날짜·소유자는 DoU홈에서 렌더하지 않지만 소개는 값만 있으면 그린다. 정보 화면·수정 화면·설정 허브 부제 세 곳에 노출하고, 값이 없거나 빈 문자열이면 행을 아예 두지 않는다. ADR-0047이 "Figma에 없다"는 이유로 배제했던 항목을 되살린 것이다.
