...
11. **1:1 전용 하단 블록과 작성기 잠금** — 상대가 나간 1:1은 스트림 맨 아래에 파생 상태 푸터를
    두고 작성기를 비활성화한다. 이 화면의 다른 stereo에는 없는 동작이므로 서술은
    [dm-chat.md](./dm-chat.md)가 소유한다 — 여기서는 `MessageInput`의 `disabled`와 스트림 최하단
    슬롯을 그 기능이 쓴다는 사실만 남긴다.
