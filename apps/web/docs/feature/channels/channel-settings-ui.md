...
- **`useJoinMutations`(신규)** ([hooks/useJoinMutations.ts](../../../src/app/features/channels/hooks/useJoinMutations.ts), 배럴 export) —
  `useRuntimeRepositories().join.updateJoin`([JoinRepositoryV2.ts:125](../../../../../libs/data/src/repositories-v2/JoinRepositoryV2.ts))를
...
- **`resolveChannelName`** — 폐기됐다. 제목 계산은 [`lib/resolveChannelTitle.ts`](../../../src/app/features/channels/lib/resolveChannelTitle.ts)
  체인 하나로 통합됐다(ADR-0039). 당시 서술:
  `channel.$join?.nick?.trim() || channel.name?.trim() || ''`. 홈·channels 양쪽이 쓰므로 feature가 아닌 **공용 utils**에 둔다.
  `$join`은 채널 뷰에 인라인으로 실려온다([useChannelUnreads.ts:36](../../../src/app/hooks/useChannelUnreads.ts) —
