...
> 대상: `RuntimeOverlay` '디바이스' 탭(dev 오버레이) · `apps/web/src/app/features/debug` — `DebugPushPage`(`/debug/push`), `DebugPage`(Device Info)
...
같은 정보(deviceId·토큰 등록·수신)를 두 surface에서 볼 수 있다:

| surface                        | 여는 법                                          | 특징                                                                       |
| ------------------------------ | ------------------------------------------------ | -------------------------------------------------------------------------- |
| **RuntimeOverlay '디바이스' 탭** | dev 빌드 우하단 `debug` 플로팅 버튼 → '디바이스' | 인증·언락 불필요, 앱 위에 떠 있어 가장 빠름 (`VITE_ENV=DEV`\|`LOCAL`에서만) |
| **`/debug/push` 페이지**        | 앱 버전 10탭 언락 → Debug 허브 → Push            | 세션 필요, 복사·상세 UI                                                    |
...
- 디버그 언락(`/debug` 페이지용): 앱 버전 텍스트를 3초 내 10번 탭. (RuntimeOverlay는 dev 빌드에서 언락 없이 열린다.)

## ① deviceId 확인

`/debug` 상단 **Device Info** 블록에서 `Device ID`(= `window.CHATIC_APP_DEVICE_ID` = 네이티브 `DeviceInfo.getUniqueId`와 Firebase installation id를 이은 `deviceId:firebaseInstallId` 조합), `Install ID`, `Platform`을 본다. 각 행을 탭하면 클립보드로 복사된다.

> `deviceToken`은 여기 없다 — 네이티브가 글로벌로 주입하지 않으므로 Push 페이지에서 브릿지로 조회한다.

## ② deviceToken 서버 등록여부

`/debug/push` → **Server Registration** → `Check`.
...
| 앱 상태          | 도달 경로                                    | 관측 위치                                             |
| ---------------- | -------------------------------------------- | ----------------------------------------------------- |
| 포그라운드       | 네이티브 → 브릿지 `OnReceiveNotification`     | `/debug/push` **Received** 목록 + Log Buffer(`PUSH`)  |
