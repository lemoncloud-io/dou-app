...
- 클라우드 유도 배너 — 중계 홈 상단. 소유 클라우드 0개일 때만, 닫기 24시간 유지.
...
2. **클라우드 진입** — `selectedCloudId !== 'default'`. 헤더 좌측 = `CloudAvatar`(이름 이니셜) + 클라우드
   ...
   (클라우드를 이미 보유).
3. **배너 → 구독 유도** — 중계 홈 배너의 `클라우드 추가 >` 탭 → `SubscriptionSelectDialog`(플랜 선택) →
   `EmailVerifyDialog`(이메일 → 인증코드) → IAP. 안내 화면을 경유하지 않는다.
...
- **두 위치의 차이** — 홈은 액션 링크(`클라우드 추가 >`)와 닫기를 모두 쓰고, 시트는 닫기만 쓴다(시트에는
  섹션 footer의 `＋ 클라우드 추가`가 이미 있다). 래퍼 컴포넌트는
