...
import { readCloudUnreadSnapshot, writeCloudUnread, type SnapshotStorage } from './cloudUnreadSnapshot';
...
describe('readCloudUnreadSnapshot', () => {
    ...
    it('깨진 JSON은 빈 맵으로 안전하게 처리한다', () => {
        const broken = createStorage({ 'testbed-cloud-unread': '{not json' });
...
describe('writeCloudUnread', () => {
    ...
    it('count 0이면 해당 cid 항목을 제거해 맵을 희소하게 유지한다', () => {
        writeCloudUnread('cloud-1', 3, storage);
    ...
    it('다른 클라우드 항목은 보존한다', () => {
        writeCloudUnread('cloud-1', 3, storage);
        const next = writeCloudUnread('cloud-2', 5, storage);
        expect(next).toEqual({ 'cloud-1': 3, 'cloud-2': 5 });
    });

    it('값이 그대로면 storage에 다시 쓰지 않는다(불필요한 갱신 방지)', () => {
        writeCloudUnread('cloud-1', 3, storage);
        expect(storage.writes).toBe(1);
        const again = writeCloudUnread('cloud-1', 3, storage);
        expect(storage.writes).toBe(1); // no redundant write
        expect(again).toEqual({ 'cloud-1': 3 });
    });
});
