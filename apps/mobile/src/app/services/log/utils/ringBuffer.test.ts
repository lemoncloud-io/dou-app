import { createRingBuffer } from './ringBuffer';

describe('ringBuffer', () => {
    it('push 후 peek은 FIFO 순서로 조회되어야 한다', () => {
        const q = createRingBuffer<number>(2);

        q.push(1);
        q.push(2);
        q.push(3);

        expect(q.size()).toBe(3);
        expect(q.peek()).toEqual([1, 2, 3]);
    });

    it('shift는 앞에서부터 제거되어야 한다', () => {
        const q = createRingBuffer<number>(2);
        q.push(10);
        q.push(20);
        q.push(30);

        const firstTwo = q.shift(2);

        expect(firstTwo).toEqual([10, 20]);
        expect(q.size()).toBe(1);
        expect(q.peek()).toEqual([30]);
    });

    it('load는 전달된 배열로 큐를 재구성해야 한다', () => {
        const q = createRingBuffer<string>(2);
        q.push('a');
        q.push('b');

        q.load(['x', 'y', 'z']);

        expect(q.size()).toBe(3);
        expect(q.toArray()).toEqual(['x', 'y', 'z']);
    });

    it('clear는 큐를 완전히 비워야 한다', () => {
        const q = createRingBuffer<number>(4);
        q.push(1);
        q.push(2);

        q.clear();

        expect(q.size()).toBe(0);
        expect(q.peek()).toEqual([]);
    });

    describe('고정 크기 링 버퍼 (maxCapacity)', () => {
        it('maxCapacity를 초과하면 가장 오래된 요소를 덮어써야 한다', () => {
            const q = createRingBuffer<number>(2, 3); // 초기 용량 2, 최대 용량 3

            q.push(1);
            q.push(2);
            q.push(3);
            expect(q.size()).toBe(3);
            expect(q.toArray()).toEqual([1, 2, 3]);

            q.push(4); // 1을 덮어써야 함
            expect(q.size()).toBe(3);
            expect(q.toArray()).toEqual([2, 3, 4]);

            q.push(5); // 2를 덮어써야 함
            expect(q.size()).toBe(3);
            expect(q.toArray()).toEqual([3, 4, 5]);
        });

        it('shift는 고정 크기 버퍼에서도 앞에서부터 제거되어야 한다', () => {
            const q = createRingBuffer<number>(2, 3);
            q.push(1);
            q.push(2);
            q.push(3);
            q.push(4); // [2, 3, 4]

            const shifted = q.shift(1); // 2를 제거

            expect(shifted).toEqual([2]);
            expect(q.size()).toBe(2);
            expect(q.toArray()).toEqual([3, 4]);
        });

        it('load는 고정 크기 버퍼의 maxCapacity를 존중해야 한다', () => {
            const q = createRingBuffer<number>(2, 3);
            q.push(1);
            q.push(2);

            q.load([10, 20, 30, 40, 50]); // 5개 아이템 로드 시도, maxCapacity는 3

            expect(q.size()).toBe(3);
            expect(q.toArray()).toEqual([30, 40, 50]); // 최신 3개만 유지되어야 함
        });

        it('clear는 고정 크기 버퍼를 비워야 한다', () => {
            const q = createRingBuffer<number>(2, 3);
            q.push(1);
            q.push(2);
            q.push(3);

            q.clear();

            expect(q.size()).toBe(0);
            expect(q.toArray()).toEqual([]);
        });
    });
});
