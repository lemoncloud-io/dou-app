...
describe('MetaDataSource Test', () => {
    ...
    it('save & fetch: 페이징 스냅샷 저장 시 uid scope로 분리되어야 한다', async () => {
        const data = { ids: ['1', '2'] };
        await metaDataSource.save('chat', 'cloud_1', 'user_1', 'query_key', data);

        const result = await metaDataSource.fetch('chat', 'cloud_1', 'user_1', 'query_key');
        expect(result?.ids).toEqual(['1', '2']);
        expect(result?.uid).toBe('user_1');
    });

    it('clear: 특정 도메인(type)의 데이터만 선택적으로 초기화해야 한다', async () => {
        await metaDataSource.save('chat', 'c1', 'u1', 'k1', { ids: [] });
        await metaDataSource.save('site', 'c1', 'u1', 'k2', { ids: [] });

        await metaDataSource.clear('chat'); //

        expect(await metaDataSource.fetch('chat', 'c1', 'u1', 'k1')).toBeNull();
        expect(await metaDataSource.fetch('site', 'c1', 'u1', 'k2')).not.toBeNull();
    });
});
