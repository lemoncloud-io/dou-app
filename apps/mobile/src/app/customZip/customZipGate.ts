 ...
 * ADR-0080 decision 13 removed the web-address switcher for. The app's `FloatingMenu` gated the feature
 * with exactly this check — independent of the debug unlock — and moving the controls to the web
 * (decision 11) moves the guard here rather than dropping it.
 *
 * Reads the **baked** `VITE_ENV`, which no web page can set: gating on anything the web supplies
 * would let a compromised bundle unlock the thing being gated (the reason decision 5 split
