...
>
> **정합성 검증 엔트리도 이 파이프를 탄다.** 일치해야 하는 두 값이 어긋난 것을 클라이언트가 직접 판정해
> `warn` 한 줄로 남긴다 — 실패가 없어 실패 지향 트리거로는 잡히지 않는 계열(뱃지 대 안 읽음, 나간 멤버,
> 낡은 표시 이름)을 위한 것이고, 판정을 클라이언트가 하는 이유는 저장된 로그를 서버가 값으로 필터할 수
> 없기 때문이다. [정합성 검증 트리거](./divergence-checks.md) 참고.
>
> **태그는 열린 계약이지만 이제 확인은 된다.** 카탈로그가 정의한 태그가
> [`KNOWN_LOG_TAGS`](../src/core/tags.ts)에 있고 `LogTag`는 그 리터럴 union에 `string`을 더한 열린
> 타입이다 — ADR-0047이 닫힌 union을 걷어낸 이유(구버전 셸의 태그가 브릿지를 그대로 건너야 한다)는
> 그대로 두고, 오타만 눈에 띄게 한다.
>
> **삼켜진 실패와 버려진 데이터도 이 파이프를 탄다.** 배경 동기화의 연속 실패, 클라우드 전환 중 버려지는
> 캐시 쓰기, 그리고 **이 큐 자신의 축출**이 그것이다 — 마지막 것은 큐 밖의 제3자가 읽어 남기는데, 큐도
> 업로더도 자기 실패를 로그로 만들 수 없기 때문이다(원칙 3·4). [조용한 실패·유실 트리거](./silent-loss-triggers.md) 참고.
...
**제외**: (0066) 앱이 서버로 직접 전송(인증 클라이언트 부재 — ADR-0066 대안 참조) · 구버전 앱 폴백 · 앱→웹 flush 지시 메시지 · 낱건 sender의 스로틀(실측 후 판단 — `열린 항목` A). (기존) [triggers.md] 카탈로그 전면 적용(후속 트랙 — 이 개정은 운반 장치만 만든다) · ~~자동 error 리포트 제거~~(**2026-09에 완료** — `reportError` 삭제, [apps/web logging](../../../apps/web/docs/architecture/logging.md)의 "리포트와 로그의 관계" 참조) · 로그 보존·삭제 정책(서버 미정) · 동적 원격 로그 제어(스위치를 읽는 자리까지만) · P3 소스맵 심볼리케이션(이 트랙 밖에서 별도로 해결됐다 — error-reporting.md의 "minified 스택 읽기") · fingerprint 개선 · breadcrumb redact 정책 변경 · OS 전체 로그 스트림 · `apps/desktop` 일체.

## 시나리오

**S1 — 웹 단독에서 에러 발생.** uncaught 예외 → [app.tsx](../../../apps/web/src/app/app.tsx)의 전역 핸들러가 `logger.error('GLOBAL', '[window.onerror] …', { error, data })`로 큐에 적재(발생 시각 스탬프). **그게 전부다** — 2026-09에 자동 에러 리포트(`reportError`)가 폐지돼, 에러는 다른 엔트리와 구별 없이 업로더를 탄다. 경로 구분은 메시지 접두사로만 남는다.

**S2 — 하이브리드(WebView)에서 에러 발생.** S1과 동일하다. 엔트리는 `SendLog`(timestamp·원본 tag·`source:'web'`)로 네이티브 통합 버퍼에 합류하고, 거기서부터 네이티브 발원 엔트리와 구별되지 않는다. 리포트 쪽 장치(`LogSource`·통합 버퍼 pull·tail 50 첨부)는 첨부 폐지(2026-08-21)와 `reportError` 폐지(2026-09)로 차례로 사라졌다.
...
에러 감지의 시퀀스(S2 기준). `reportError` 폐지 후에는 **두 번째 경로가 없다** — 감지는 엔트리 하나를 남기고 끝나고, 서버로 나가는 것은 업로더의 주기가 결정한다:

```mermaid
sequenceDiagram
    participant H as 전역 핸들러 (apps/web)
    participant L as 코어 logger
    participant Q as 미전송 큐
    participant N as 네이티브 통합 버퍼
    participant U as 업로더 (주기)
    participant S as /hello/report-bulk

    H->>L: logger.error('GLOBAL', '[window.onerror] …', {error, data})
    L->>Q: publish → 적재 (발생 시각·id·컨텍스트 스탬프)
    L->>N: SendLog (timestamp·tag·source 보존, 하이브리드만)
    Note over U: 다음 tick까지 대기 — error 앞당김은 ADR-0066에서 제거됐다
    U->>Q: peek(batchSize)
    U->>S: POST (평탄 리스트)
    S-->>U: 2xx → ack(ids)
```
...
| [`redaction/`](../src/redaction)         | [sensitiveKeys.ts](../src/redaction/sensitiveKeys.ts) · [valuePatterns.ts](../src/redaction/valuePatterns.ts) · [redact.ts](../src/redaction/redact.ts)                                                                           | "무엇이 비밀인가"를 두 축으로 판정한다 — **이름**(`SENSITIVE_KEYS`·`isSensitiveKey`·`isSensitiveField`, 과잉 마스킹을 막는 `SAFE_KEYS` 허용목록과 boolean 존재 플래그 예외 포함)과 **모양**(`redactText` — **URL 쿼리 값**을 이름만 남기고 가린 뒤, 이메일·JWT·Bearer·FCM 토큰·전화번호·AWS 키를 각자의 자리표시자로). 쿼리 축이 있는 이유는 이 앱이 쿼리스트링에 capability 재료를 싣기 때문이다(`routeTrail`의 보안 주석) — 정책은 `report/reportUrl.ts`가 리포트용으로 이미 써 둔 것과 같고, 로그 쪽은 `code`조차 보존하지 않는 점만 더 엄격하다. 그 위에 "어떻게 가리는가"(`redactSensitive`·`redactMaybeJson`·`truncate`). 모양 축이 있는 이유는 이름이 없는 자리 — 자유 문장인 `message`, 키가 아무 말도 안 하는 문자열 값 — 가 실제 유출 지점이기 때문이다. 카탈로그 금지 목록이 **경계에서** 지켜지는지는 [forbiddenContent.spec.ts](../src/redaction/forbiddenContent.spec.ts)가 본다 — 조각이 다 맞아도 배선이 한 칸 틀리면 값은 그대로 나간다                      |
| [`serialization/`](../src/serialization) | [safeStringify.ts](../src/serialization/safeStringify.ts) · [safeSerializable.ts](../src/serialization/safeSerializable.ts) · [serializeLogs.ts](../src/serialization/serializeLogs.ts) · [wire.ts](../src/serialization/wire.ts) | 로그 값을 안전하게 만드는 두 형태 — **문자열로 평탄화**(저장·전송용)와 **구조 유지**(브레드크럼·디버그 UI용, axios 에러는 필드별로 들어냄). 마스킹 범위는 다르다 — 앞은 전 키를 순회하고, 뒤는 자기가 분해하는 axios 요청/응답 상세만 가린 뒤 나머지는 저장·전송 경계(`serializeLogs`·`toWireLogEntry`)에 맡긴다. 그 위에 `SerializedLog`(예산 40k/2k)와 서버 wire 매핑 — **둘은 같은 형태다**: `serializeLogs`가 `toWireLogEntry`로 평탄화하고 총 예산만 얹는다. 예전에는 저장 쪽이 `level`·`tag`·`message`·`timestamp` 넷만 남겨 **컨텍스트 열 개와 `id`를 버렸고**, 그래서 프로세스보다 오래 산 엔트리(= 죽은 런의 엔트리)가 사용자·실행·버전·화면 없이 서버에 도착했다. `id`가 없으면 호스트의 ack도 서버 dedup도 성립하지 않아 매 주기 재전송된다. 필드 절단 규칙은 `truncateText`로 한 벌만 둔다. **`message`는 `safeStringify`를 타지 않으므로**(이미 문자열이라 그대로 필드로 들어간다) `serializeLogs`·`toWireLogEntry`가 각자 `redactText`를 부른다 — 절단 **전에** |
...
**경계가 고정하는 성질** (`libs/bridges/src/logger/` — `setupBridgeLogger.spec.ts` · `appLogInfoCodec.spec.ts`)

`appLogInfoCodec`의 `LogEntry` ↔ `AppLogInfo` 왕복이 컨텍스트 열 개를 잃지 않는지. 그리고 낱건 sender가 `debug`를 브릿지로 보내지 않으면서 hub에는 발행하는지 · 엔트리 하나가 메시지 하나인지
