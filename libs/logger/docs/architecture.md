...
`perf/`는 상태를 클래스가 아니라 **모듈 변수**로 든다(②의 규칙에 대한 유일한 의도적 예외). 호출자가 사이트 전환·웹바이탈 콜백처럼 관계없는 코드에 묻힌 계측 지점이라 인스턴스를 넘겨받을 수 없고, 클래스로 만들어도 결국 싱글턴 홀더가 필요하다. 드는 것은 카운터와 `Logger` 참조뿐이고 엔트리가 아니므로 "저장소는 하나"는 그대로다. 자세한 것은 [성능 예산과 지표 이벤트](./perf-metrics.md).
