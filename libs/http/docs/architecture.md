...
- `gateways/`(oauth·users·clouds·subscriptions 경로·메서드 소유) — 2단계에서
  ...
  ([HTTP 데이터 경로](../../data/docs/remote/http.md) 문서 소관)
- `calcSignature`(lemon HMAC, awsSigning.ts:25)
  — 3단계 `@chatic/auth-sign` 신설 시 이관([auth-sign README](../../auth-sign/README.md)).
...
- ~~`logBatch`·`reportIssue`의 실제 이관~~ — **2026-09-02에 들어왔다.** 와이어 어휘는
  ...
  `data`의 `report` repository를 거친다([HTTP 데이터 경로](../../data/docs/remote/http.md)).
...
2. **relay 서명 요청** — `executeSignedRelayRequest`. lemon adapter의 `buildSignedRequest`가
   lemon 자체 저장소의 signing material로 서명한다.
    ...
    > 않았다** — 리포 전체 소비가 0이어서다([http.md의 REST hook consumers](../../data/docs/remote/http.md#rest-hook-consumers)가
    > 같은 실측을 담고 있고, `apps/admin-v2`의 주석 한 줄만 그 엔드포인트를 언급한다).
