...
export interface IInviteCloudRepository extends ILocalCacheMutationRepository<CacheCloudView> {
...
export class InviteCloudRepository extends BaseRepository implements IInviteCloudRepository {
    ...
    public cacheCreate(item: Partial<CacheCloudView>): Promise<void> {
        const id = item.id || '';
        return this.inviteCloudLocalDataSource.saveInviteCloud(id, item);
    }

    /** 로컬 캐시의 초대 cloud 일부 필드를 갱신합니다. (remote 호출 없음) */
    public cacheUpdate(id: string, patch: Partial<CacheCloudView>): Promise<void> {
        return this.inviteCloudLocalDataSource.updateInviteCloudPartial(id, patch);
    }
    ...
    public async cacheBulkCreate(items: Array<Partial<CacheCloudView>>): Promise<void> {
        await Promise.all(
            items
                .filter(item => !!item.id)
    ...
    public async cacheBulkUpdate(items: Array<LocalCacheBulkPatch<CacheCloudView>>): Promise<void> {
        await Promise.all(
            items
                .filter(item => !!item.id)
