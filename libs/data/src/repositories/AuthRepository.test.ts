...
describe('AuthRepository', () => {
    const createRepository = () => {
        ...
        const repository = new AuthRepository(authSocketDataSource as any, contextProvider as any);

        return { repository, authSocketDataSource };
    };

    it('sendPhoneCode delegates the number and options unchanged', async () => {
    ...
    it('verifyPhoneCode delegates mode and countryCode unchanged', async () => {
        const { repository, authSocketDataSource } = createRepository();
    ...
    it('verifyPhoneCode returns linkable=false as a response rather than turning it into an error', async () => {
        const { repository, authSocketDataSource } = createRepository();
        ...
        // The blocking reason arrives in a response slot only on verify — confirm throws 409/403 for the same situation.
        expect(result).toBe(blocked);
    });

    it('a session-switch token is returned as is, uninterpreted', async () => {
        const { repository, authSocketDataSource } = createRepository();
        const confirmResult = {
            step: 'confirm',
            mode: 'login',
        ...
        // Installing the token belongs to app-runtime's session/auth (libs/app-runtime/docs/session/architecture.md).
        expect(result).toBe(confirmResult);
    });

    it('verifySocialAccount delegates the token bundle unchanged', async () => {
        const { repository, authSocketDataSource } = createRepository();
    ...
    it('confirmSocialAccount delegates the token bundle unchanged', async () => {
        const { repository, authSocketDataSource } = createRepository();
    ...
    it('a remote failure is not swallowed but rejected as is', async () => {
        const { repository, authSocketDataSource } = createRepository();
...
describe('AuthRepository — HTTP account/alias/invite surface (ADR-0070 late stage 2)', () => {
    ...
    it('throws a clear error when IAuthHttpDataSource is not injected', async () => {
        const repository = new AuthRepository({} as any, contextProvider as any);
    ...
    it('delegates to the injected http data source', async () => {
        ...
        const repository = new AuthRepository({} as any, contextProvider as any, http as any);
