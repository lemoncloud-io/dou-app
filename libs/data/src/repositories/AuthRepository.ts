...
export interface IAuthRepository extends DisposableRepository {
    /**
     ...
     * "session/auth — HTTP goes through the data layer"). `data` performs the call; it never reads a
    ...
    /**
     * HTTP account/alias/invite commands (ADR-0070 decision 5, late stage 2). `IAuthHttpDataSource`
     * injection is optional through stage 2 — every existing `createRepositories` call site stays
     * green without it, and these methods throw a clear "not wired yet" error until it's injected.
     * Stage 4 promotes it to required once the REST hooks actually move behind it.
     */
    ...
    /**
     * Session-material commands (`session/auth`'s only HTTP path).
     *
...
/** The body type for `exchangeToken` — pulled straight from the gateway contract. */
 ...
 * entity as `link$` and is served by UserRepository's cache (ADR-0042 §5).
 *
 * Kept apart from UserRepository deliberately: that one already composes four data sources and owns
...
export class AuthRepository extends BaseRepository implements IAuthRepository {
    ...
    private requireHttp(): IAuthHttpDataSource {
        if (!this.authHttpDataSource) {
            throw new Error('[AuthRepository] IAuthHttpDataSource is not injected — httpFactory not wired yet.');
        }
