...
[libs/data의 local 문서](../../data/docs/local/README.md#채널-한정-삭제의-세-경로)가 이것을 테스트
...
코드는 리포에 없고, `SyncMetaLocalDataSourceV2`가 저장된 `expiresAt` 대신 `lastSyncedAt`으로 현재
...
- **`loadMany`는 결과의 길이도 순서도 보장하지 않는다.** 없는 id는 빠진다. 반드시 id로 다시 색인해야 한다(`BaseLocalDataSourceV2.indexById`). `existing[index]`로 짝을 맞추면 캐시에 없는 항목 하나 때문에 그 뒤 전부가 남의 기존 행과 병합된다.
...
이유)는 [libs/data의 local 문서](../../data/docs/local/README.md#채널-한정-삭제의-세-경로)가 정본이다.

**프로덕션 호출자는 둘이다** — 채널 퇴장 purge(`ChannelRepositoryV2.leaveChannel`)와 소켓 sync의
...
- `repositories/ChannelRepositoryV2.ts` — 퇴장 purge 실패 경고(ADR-0067)
