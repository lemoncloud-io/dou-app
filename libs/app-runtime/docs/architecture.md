...
| ⑥   | 사이트 전환                        | `web-core` (`hooks/session/actions`)              | cloud/relay site session refresh → `activeServer.siteId` 갱신 → data context 갱신. 현재는 새 socket 생성보다 기존 socket의 `auth:update` 재갱신 성격에 가깝고, 이 단계가 빠지면 서버는 이전 site 세션을 계속 볼 수 있다 |
