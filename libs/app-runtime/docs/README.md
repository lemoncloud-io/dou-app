...
| [auth/](./auth/README.md)       | SDK `AuthController`(`client.auth`) 소유·표면·부팅/재인증 배선               | [usage.md](./auth/usage.md) — 배선/사용 패턴·트러블슈팅 · [signing.md](./auth/signing.md) — per-kind authId/sign/writeback 계약                                                                                         |
...
- 같은 사실은 한 곳(정본)에만 쓰고 나머지는 링크한다 — 정렬 상태·소유 규칙(manager 2축 + SDK 인증)은 [architecture.md](./architecture.md), plan 패밀리는 [sync/library-internals.md](./sync/library-internals.md), 게이트웨이 타입은 [sync/gateway-reference.md](./sync/gateway-reference.md).
