...
| 부팅 전 정책 주입  | `configureDataRuntime({ repositories?, cache? })`              | 데이터 런타임 생성 **전** 등록(늦으면 경고 후 무시). 호출은 병합되므로 두 정책을 따로 등록해도 된다 — apps/web은 `repositories`, desktop-web은 `cache`(채널당 chat 상한) |
