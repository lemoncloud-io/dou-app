...
Date: 2026-06-29
Status: viewing 통지 구현됨 (status 트리거·다른 디바이스 구독은 미구현)
...
| **내 상태 변경** (status·posX·posY·name) | `saveDevice(body)`                       | **+1**   | `DeviceView`          | 매 save가 서버에서 `status='green'` 강제 |
...
| **viewing 통지 트리거** (채널 진입/이탈 → `syncDevice`)       | ✅ **배선됨**             | apps/web `useDeviceViewing`(라우트 관측) → `DeviceRepositoryV2.notifyViewing` → `syncDevice` (§4)               |
| **상태 변경 트리거** (`saveDevice({status})`)                 | 🔴 **없음**(자동 save 외) | —                                                                                                               |
| device watch 결과 캐시 (`registerDevice` → repository)        | 🟡 미연결                 | plan `onUpdate`가 캐시에 안 붙음([usage.md](usage.md) §2 "device: 캐시 미연결")                                 |

→ **viewing 통지 트리거는 배선됐다**(§4). 남은 빈칸은 "내 상태(status/위치)를 쏘는" 트리거와 "남의 디바이스 상태를 받는" watch 캐시 연결(§5)이다.

---

## 4. viewing 통지 surface (구현됨)
...
```ts
// 채널 진입
notifyViewing('channel', channelId); // → syncDevice({ viewingType:'channel', viewingId: channelId })

// 채널 이탈 / 목록 복귀 (clear)
notifyViewing('', ''); // → syncDevice({ viewingType:'', viewingId:'' })
```

규칙:

1. **`syncDevice`만 쓴다**(save 아님) — tick 불변·fire-and-forget. 실패해도 다음 전환에서 갱신된다.
2. **항상 짝으로** — `viewingType`/`viewingId`를 함께 set 또는 함께 clear.
3. **`viewingType`은 LUT 제한** — 시작은 `'channel'`뿐. `viewingId`는 대상 id(채널이면 channelId).
4. **연결 끊긴 동안의 위치는 통지 안 됨** — 서버가 연결 활성 여부와 함께 해석한다.

### 4.3 상태 변경(status/position)이 필요하면

- `saveDevice({ status })` / `saveDevice({ posX, posY })` — **request/response, tick +1**.
- 고빈도 입력(마우스 좌표 등)은 **throttle / coalesce** 필수. 매 픽셀 save = tick 폭증.
- 단순 "온라인 표시"는 별도 save 불필요 — connect 자동 save가 이미 `status='green'`을 만든다.
...
- **내 상태(status/위치)** → `saveDevice` (tick +1, throttle) — 트리거 미구현
...
남은 작업 = (1) 내 status/위치 save 트리거(필요 시), (2) device watch 결과 캐시 연결(§5).
