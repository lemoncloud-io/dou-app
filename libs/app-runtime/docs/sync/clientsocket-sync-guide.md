# ClientSocket Sync 메커니즘 분석 및 클라이언트 동기화 지침

Date: 2026-06-23

## 0. 이 문서의 범위

이 문서는 `@lemoncloud/chatic-sockets-lib`(소스 리포: `chatic-sockets-api`)의 **v2 클라이언트 동기화 계층**을 코드 기준으로 분석하고, `libs/app-runtime` / `libs/data`가 이 라이브러리를 **어떻게 소비해 동기화해야 하는지** 지침과 예제를 제공한다.

- 본문에 등장하는 `src/...` 경로는 **라이브러리 소스 리포(`chatic-sockets-api`)** 기준이다. 이 리포(`chatic-front`)의 경로가 아니다.
- 동기화 시점·orchestration 정책은 [sync/README.md](README.md)(Sync Domain Spec)가 소유한다. 이 문서는 그 아래에서 도는 **라이브러리 계층의 실제 동작**을 설명하는 레퍼런스다.
...
## 7. 게이트웨이 요청/응답 레퍼런스 (deprecated 제외)

각 게이트웨이 메서드의 **요청 입력**과 **실제 응답 타입**이다.
...
### 7-B. device (`createDeviceGateway`)
...
### 7-C. chat (`createChatGateway`) — 응답 `$socials`
...
### 7-D. channel (`createChannelGateway`) — 응답 `$socials`
...
### 7-E. profile (`createProfileGateway`) — 응답 `$socials`
...
### 7-G. cloud (`createCloudGateway`) — 응답 `$backend`
...
### 7-H. user (`createUserGateway`) — 응답 `$socials`/`$backend`
