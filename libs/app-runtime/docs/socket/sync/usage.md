...
`join`의 `onRemove`는 캐시 툼스톤 하나만 남기지 않는다. 사라진 행이 **내 것이면** 그 채널의 chat 캐시까지 비운다 — 강퇴와 타 기기 퇴장은 `leaveChannel`을 거치지 않으므로 이 신호가 유일한 통로다. 삭제는 되돌릴 수 없어서 `dropForeignFrame()`으로 한 번 더 거른다(자기 클라우드보다 오래 산 소켓의 프레임이 현재 파티션을 겨누면 안 된다). 툼스톤은 그 게이트 앞에서 기존대로 남긴다. 근거는 ADR-0067.
