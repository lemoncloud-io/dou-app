...
**active 슬롯**(cloud 있으면 cloud, 없으면 relay)으로 나간다([SocketManager.ts:239](../../src/socket/SocketManager.ts),
[types.ts:59-65](../../src/socket/types.ts)). 하지만 어떤 요청은 클라우드 활성 여부와 무관하게 **특정 서버 슬롯**으로만 가야 한다.
...
- **지연 해석 필수.** 슬롯은 `ensure()`에서 teardown/rebuild되므로([SocketManager.ts:91-118](../../src/socket/SocketManager.ts)),
...
4. `DeviceSocketDataSource.updateRemoteDevice`가 relay 파사드의 device gateway로 `device.update-remote { muted: true }`를 **relay 슬롯**으로 전송(고정).
...
- [`repositories/DeviceRepositoryV2.ts`](../../../../libs/data/src/repositories-v2/DeviceRepositoryV2.ts) —
...
- **코어 단위 테스트** — [`SocketManager.test.ts`](../../src/socket/SocketManager.test.ts) "getScopedClient (kind-scoped routing)"
...
- **라우팅/도메인 테스트** — [`DeviceSocketDataSource.test.ts`](../../../../libs/data/src/remote/socket-data-sources/DeviceSocketDataSource.test.ts)
    - [`DeviceRepositoryV2.test.ts`](../../../../libs/data/src/repositories-v2/DeviceRepositoryV2.test.ts):
