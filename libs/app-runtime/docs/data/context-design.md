...
- 전역 context는 [`useRuntimeBinding`](../../src/runtime/useRuntimeBinding.ts)이 `session.activeServer`·`cloud`·`identity`로부터 계산한다 — `cid`는 선택된 cloud(optimistic), `sid`=`activeServer.siteId`, `uid`=`identity.userId`. (프로필 파생값은 web-core가 아니라 app 레이어 `useRuntimeProfile`이 계산한다.)
...
- **`dropForeignFrame`** (sync plans) — sync callback도 socket `boundCid` ≠ data context cid인 frame을 버린다([../sync/README.md](../socket/sync/README.md)).
...
- [../sync/README.md](../socket/sync/README.md) — sync frame의 cross-cloud 가드
