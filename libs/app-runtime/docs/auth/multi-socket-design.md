...
| apps/web·testbed **병렬 리프레시 전체**(`useTokenRefresh` 주기 루프 + 부팅 `refreshToken`의 relay/cloud 레그, `refreshActiveCloudSession` 포함) | ✅ **제거(Phase 2, 2f/2g)** | 토큰 refresh는 이제 **소켓별 SDK가 소유**(§2-0) → SDK 경로(apps/web)에서 web-core 병렬 HTTP 리프레시는 **더 이상 사용 안 함**. 주기 루프는 `skipPeriodicRefresh:true`로 이미 off이나, 부팅 `initialize()`가 아직 병렬 refresh를 1회 돌림. 제거 전 **부팅 프로필 하이드레이션 → app-runtime `useSessionProfile`**, **하드 만료 로그아웃 → SDK `expired`+`onAuthExpired`** 로 소유자 이전을 확인. admin/desktop-web은 함수·루프 **존치**(§2-4) |
...
> `refreshRelaySession`/`refreshCloudSession`/`refreshActiveCloudSession` **함수 자체는 web-core에 존치**한다 — admin/desktop-web이 `useTokenRefresh`(병렬 리프레시)를 통해 계속 사용하므로(§2-4). 제거되는 것은 **apps/web(SDK 경로)에서의 병렬 리프레시 호출**이지 함수가 아니다.
