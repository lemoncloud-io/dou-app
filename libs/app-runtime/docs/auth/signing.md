...
## 2. per-server web-core 헬퍼 (delegate가 kind로 호출)
...
- relay/cloud 각각에서 `getServerAuthRegistration(kind)`의 `authId`·`token` 출처가 맞는가 (relay=relayCore/webTransport, cloud=cloudCore).
