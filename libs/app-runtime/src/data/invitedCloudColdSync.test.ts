import { reconcileInvitedCloudsIntoCold, recoverInvitedCloudIfMissing } from './invitedCloudColdSync';

// Isolate the two pure orchestration functions from the React hook's dependencies.
jest.mock('../runtime', () => ({ useRuntimeRepositories: jest.fn() }));
...
describe('invitedCloudColdSync', () => {
    ...
    describe('reconcileInvitedCloudsIntoCold', () => {
        ...
        it('recovers a registry entry that is absent from the cached list', async () => {
            const cloud = createCloud();
    ...
    describe('recoverInvitedCloudIfMissing', () => {
        ...
        it('re-derives and writes a missing cloud', async () => {
            const cloud = createCloud();
            cloud.cacheRead.mockResolvedValue(null);
            mockIssue.mockResolvedValue({ cloudId: 'c1', backend: 'https://b', wss: 'wss://w' });

            await recoverInvitedCloudIfMissing(cloud as any, 'c1', 'Label');
