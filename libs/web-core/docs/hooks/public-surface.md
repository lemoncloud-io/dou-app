...
- `useSessionIdentity()`
- `useSessionSelection()`
- `useCloudSessionCatalog()` (user 도메인)
- `useSiteSwitch()`
...
- ⑧⑨는 web-core hook surface가 아닙니다 — 소켓 인증 수명주기는 SDK `AuthController`(app-runtime)가 소유하고, web-core는 세션 저장소에서 **브리지 헬퍼**로 seed/sign/writeback만 공급합니다 (public-api.md "소켓 모듈 연계" 참조).
...
- **위반 (서비스 경유 필수):** 로그인·토큰 발급/갱신·클라우드 전환처럼 **세션 상태를 바꾸는 전이**. 이 hook이 `api`를 직접 부르면 세션 hydrate·저장·activeServer 재계산을 빠뜨려 일관성이 깨집니다. → `session/services` 경유. (이 이유로 과거 `useIssueToken`/`useRefreshCloudToken`/`useIssueCloudToken`가 각각 `useLogin`/`refreshCloudSession`/`useSwitchCloudSession`으로 수렴·제거됨)
