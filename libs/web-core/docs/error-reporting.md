...
- [`libs/web-core/src/api/common.ts`](../src/api/common.ts) — `reportError(error, context?: ErrorReportContext)`:
    ...
    - 요청 실패면 `http.url`/`http.method` + message 뒤에 `→ METHOD url` 노출. **message에 붙이는 것은 요청 정보뿐이다** — URL은 발생마다 고정이라 admin 그룹을 엔드포인트별로 갈라주지만(예전엔 "Network Error" 한 버킷), 좌표처럼 매번 바뀌는 값은 붙이지 않는다.
