...
> (`libs/data/src/repositories/index.ts`). 문서가 이름 붙인 `GlobalDataContextStore` /
...
- `libs/data/src/repositories/types.ts`(V1, 이후 삭제됨)와 [libs/data/src/repositories/types.ts](../../../libs/data/src/repositories/types.ts)는 repository가 provider를 통해 최신 context를 읽는 구조를 사용한다.
- [libs/data/src/local/data-sources/types.ts](../../../libs/data/src/local/data-sources/types.ts)는 이미 `contextOverride`를 지원한다.
...
- 문제와 목표가 현재 코드 구조에 직접 매핑되는가
    - 예. `RuntimeDataBinder`, `DataManager`, `BaseRepository`, `BaseLocalDataSource`를 근거로 작성했다.
