...
메시지를 **cid를 지정하거나 생략해(전체 파티션) 키워드로 검색**하고, 그 결과를 화면에 그리는 데
...
- **상한과 정렬·그룹핑은 상위 계층에서**: 어댑터/소스는 "매치 전부"를
  ...
  양쪽 동작이 어긋난다.)
- **uid는 세션에 묶인다 (호출자가 알아야 하는 한계)**: 파티션 키의 `uid`는 활성
  세션 토큰에서 파생되므로(`contextStore.ts:45-47`), 어떤 클라우드의 행은 그
  클라우드 세션의 uid로 기록된다. 따라서 `cid`를 생략한 전체 스캔은 **현재 uid로
  쓰인 행만** 돌려준다 — 계약은 크로스 파티션 스캔을 제공하지만 그것이 곧
  "모든 클라우드가 보인다"는 뜻은 아니다. apps/web 검색은 이 때문에 활성 cid를
  명시한다(ADR-0033 결정 2항). 세션과 무관한 읽기가 필요하면 `invitecloud`처럼
  글로벌 파티션을 강제하는 선례가 있다(`storages/utils.ts:66`).
...
- **컨텍스트 조회 `resolveContext`**: 명시적 cid로 채널·플레이스 행,
  내 join 행, 채널별 최신 chat, 그리고 클라우드의 표시 프로필(발신자 이름·사진)을
  읽는다. 프로필은 요청한 부분집합이 아니라 클라우드 전체를 돌려준다 — 채팅 행의
  플레이스는 소속 채널이 해석된 뒤에야 알 수 있어, 멤버를 지정해 묻는 방식은 왕복을
  두 번으로 만든다. 멤버 신원(유저 캐시)도 함께 돌려준다 — 프로필은 방을 열어야
  캐시되므로 발신자 **이름**의 폴백(계정 `name`)이 필요하다. 사진과 nick은 프로필만
  쓴다(앱 계층 규칙, [[web-search-page]] 참조). 양쪽 구현 모두 기존 배선만
...
- 클라우드 이름 검색 — 캐시 스캔이 아니라 앱 계층에서 relay 카탈로그 + 초대
  클라우드 캐시 + 캐시된 클라우드 이름으로 처리([[web-search-page]] 참조).
