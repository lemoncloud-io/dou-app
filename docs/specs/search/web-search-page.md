...
- **행 표시 정보** (모든 결과가 활성 클라우드이므로 클라우드 조각은 표시하지
  않는다 — 항상 같은 값이라 정보가 없다):
    ...
    - 발신자 이름은 방과 같은 체인을 쓴다 — 플레이스 표시 프로필 `nick` →
      유저 캐시 `nick`/`name` → 메시지에 실린 `owner$.name`. **프로필만으로는
      부족하다**: 프로필 sync는 그 방을 열어야 등록되므로 검색 결과의 방이
      미방문이면 프로필이 캐시에 없다. 아바타도 프로필 → 유저 캐시 순.
      전부 없으면 이름은 `search.unknownSender`, 아바타는 기본 메시지 아이콘.
