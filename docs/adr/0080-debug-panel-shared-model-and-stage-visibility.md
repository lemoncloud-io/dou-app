...
### 델타 ② — `platforms`가 아직 없다

`appBridge`를 부르는 화면이 8개인데 네이티브 게이트를 가진 파일은 4개다. 브라우저에서 앱 아이콘 ·
인앱결제 · OAuth · SMS · 기기 정보 · 커스텀 zip · 부팅 기록을 열면 눌러도 실패하는 버튼만 보인다.
매니페스트에 `platforms`를 넣고 셸이 배지로 그리는 것이 다음 단계다.
...
### 델타 ④ — 노출 판정이 아직 sessionStorage다

`debug.overlayEnabled`는 `byStage: { LOCAL: true, DEV: true }`로 레지스트리에 있다. 그런데 패널의
마운트 조건은 `useDebugMode`(sessionStorage + 주입 전역)이고 이 행을 읽지 않는다. **LOCAL/DEV에서
10탭 마찰이 사라진다는 결과는 아직 오지 않았다.**

관련해서 한 가지가 배선됐다. 언락이 `system.overridesUnlocked`를 함께 연다 — 이 배선이 없어서
PROD에서는 `surface: 'dev'` 키가 전부 읽기 전용으로 떴을 것이다(결정 4의 로컬 레인 전제가 무너진다).
