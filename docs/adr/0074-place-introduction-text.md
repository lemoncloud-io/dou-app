# ADR-0074: Place introduction text — reversing ADR-0095's exclusion of `desc`

> Status: Accepted · Decided: 2026-09-07
> · **Amends** ADR-0095 (0095-place-detail-read-only-screen.md) (place info screen) — cancels its §Scope exclusion of `desc`
> · Follows the owner gate established by ADR-0031 (0031-place-settings-hub.md) (settings hub)
> · Screen and implementation detail lives in [place-desc.md](../../apps/web/docs/feature/place/README.md); settings overall in [place-settings.md](../../apps/web/docs/feature/place/README.md)

## Context

When ADR-0095 stood up the place info screen, it explicitly **excluded** the introduction text
(`desc`):

> The introduction text (`desc`) is not in Figma, so it is not included. — ADR-0095 §Scope
...
5. **ADR-0095's relay exception was "a planning decision," not "missing data."** Removing the
...
This continues the rule ADR-0095 established: "don't draw a fact the server didn't give us." No
...
This diverges from the two rows ADR-0095 removed for relay. Those two rows answer "who made this
...
**Also hide it on the relay default place** — would match the shape of ADR-0095's relay exception.
...
**Edit the ADR-0095 text directly and just delete the exclusion sentence** — the smallest change.
...
- ADR-0095 remains valid, but its §Scope sentence about `desc` is replaced by this ADR. A link back
...
- **Two unverified assumptions remain** — (1) whether the `user.mysite` (list) response carries
  `desc`. ADR-0095 already saw `ownerId`/`isOwner` split between relay and cloud, so the type
