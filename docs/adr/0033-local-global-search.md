...
> 상태: Accepted · 결정일: 2026-07-29 · 갱신: 2026-08-06(검색 범위를 활성 클라우드로 축소, 플레이스 전환 확정, 컨텍스트 조회 추가)
...
2. **검색 대상 = 활성 클라우드의 로컬 캐시** (웹 IndexedDB·네이티브 SQLite 동일):
    - 플레이스, 채널, 채팅 메시지 — **활성 클라우드 파티션에 캐시된 것**.
    - **초안의 "클라우드 불문 전체 파티션"은 철회한다** (2026-08-06). 캐시는
      `(cid, uid)`로 나뉘고 `uid`는 **활성 세션 토큰**에서 파생된다
      (`contextStore.ts:45-47`) — 어떤 클라우드의 행은 그 클라우드 세션의 uid로
      쓰이므로, relay나 다른 클라우드 세션에서는 uid 필터에 걸려 보이지 않는다.
      즉 "전체 파티션 검색"은 세션에 따라 결과가 달라지는, 사용자가 예측할 수
      없는 부분 집합이었다(실사용에서 relay 전환 후 다른 클라우드 결과가 사라지는
      증상으로 드러남). 한 번에 한 클라우드가 검색 가능 범위를 정직하게 표현한다.
    - 스토리지 계약(`IGlobalCacheSearchSource`)은 크로스 파티션 스캔 능력을
      그대로 유지한다 — 호출자가 `cid`를 지정할 뿐이다. uid 축을 세션과 무관하게
      만드는 것(예: `invitecloud`처럼 글로벌 파티션 강제,
      `storages/utils.ts:66`)은 별도 결정 사항으로 남긴다.
    - 클라우드 이름 — relay 카탈로그(`useCloudSessionCatalog`) + 초대
      클라우드 캐시(`useInvitedClouds`) + 로컬 클라우드 캐시의 이름
      (`useCachedCloudNames`). 캐시 이름이 우선이며, 카탈로그가 인증·네트워크에
      의존하는 REST 읽기라 클라우드 세션이 끊기면 비는 것을 이 소스가 메운다.
      **이름 검색은 활성 클라우드에 한정되지 않는다** — 캐시 스캔이 아니라 목록
      필터이고, 다른 클라우드로 건너가는 유일한 통로다.
...
3. **단일 검색 계약, 어댑터별 이중 구현 (동작 동일성 필수)** — 스토리지
   ...
   경로는 건드리지 않는다.
    ...
    - **컨텍스트 조회도 이 소스가 담당한다** (2026-08-06 추가). 검색 결과
      ...
      훅을 일절 쓰지 않는다.** 계약과 그 근거의 전개는
      [[global-cache-search]](../specs/cache/global-cache-search.md).
4. **실시간 방출** — 키워드 입력 디바운스(300ms, 최소 2글자) 시 재스캔.
   RxJS 등 새 스트림 장치는 도입하지 않는다(리포 정석 패턴 유지).
   초안에 있던 "활성 클라우드의 플레이스/채널은 `observeList` 구독으로 라이브
   갱신을 병합한다"는 항목은 **철회한다**(2026-08-06) — 리포지토리 구독은 활성
   클라우드 파티션만 보므로 결과의 일부만 살아 있게 되어 오히려 일관성이
   깨지고, 위 3항의 "검색 결과 렌더링은 리포지토리·sync 훅을 쓰지 않는다"와
   정면으로 충돌한다. 결과는 재검색 시점의 캐시 스냅샷이다.
...
7. **메시지 검색 + 점프** (이번 범위에 전부 포함):
    ...
    - `/channels/:id/room?chatNo=` 쿼리 파라미터 신설, room 페이지에서
      파싱해 점프 스토어로 전달.
    - `useChats`에 과거 방향 반복 로드 협조(`loadMore` 예산 루프). `data-chat-no`
      스크롤 + 하이라이트는 `useChatScroll`이 아니라 별도 훅
      (`useMessageJump`)이 소유하고, `useChatScroll`은 점프 대기 중 하단 자동
      스크롤을 멈추는 역할만 맡는다 — 두 스크롤 주체가 동시에 살아 있으면 항상
      하단 고정이 이긴다([[message-jump]](../specs/search/message-jump.md)).
...
- 활성 클라우드의 플레이스/채널/메시지를 네트워크 없이 즉시 검색. 다른
  클라우드는 이름으로 찾아 전환한 뒤 검색한다(세션에 따라 결과가 달라지지 않음).
...
- **검색 결과의 신선도 미보장** — 활성 클라우드라도 캐시는 마지막 동기화
  시점의 스냅샷이다. 이름 변경·삭제된 채널이 결과에 나올 수 있다(진입 실패 처리).
- **다른 클라우드는 한 번에 검색되지 않는다** — 클라우드를 전환해야 그
  클라우드가 검색된다. 위 2항의 uid 파티셔닝이 근본 원인이며, 해소하려면 캐시
  스코프 자체를 손대야 한다.
...
**이 결정을 구현한 스펙 문서** (현재 동작의 기준 — 이 ADR은 "왜"만 담는다):

- [[global-cache-search]](../specs/cache/global-cache-search.md) — 크로스 클라우드
  캐시 읽기 계약(`search` / `resolveContext`), 웹·네이티브 구현과 공유 계약 테스트
- [[web-search-page]](../specs/search/web-search-page.md) — `/search` 화면, 결과 행
  표시 모델, 클라우드·플레이스 전환
- [[message-jump]](../specs/search/message-jump.md) — `?chatNo=` 커서 이동과
  하단 고정과의 배타 규칙
