...
> **철회 (2026-07-31)** — 잔재가 아니었다. 1:1도 플레이스 카드를 그린다.
> 게이트를 방 종류에서 **데이터 유무로 되돌렸고**(`placeName || placeIntro || placeThumbnail`),
> `RelayInviteAccept`가 `site$`를 넘긴다. 초대는 어느 쪽이든 **플레이스로** 들어가는 것이므로
> "1:1 대화에 플레이스는 의미가 없다"는 전제가 틀렸다. 같은 노드가 유효시간을 `HH:mm:ss` 한
> 줄로만 그리는 것도 아래 결정 3이 남긴 열린 질문("24시간 이상 구간의 문구는 디자이너 확인
> 필요")의 답이다 — 디자이너는 **유효기간 1일**을 전제했다.
>
> 다만 relay `invite.get`이 `site$`를 채워주는지는 확인되지 않았다. 채워질 때까지 카드는 조용히
> 접힌다(ADR-0033 D1 인터페이스 선반영). 초대자의 **플레이스 프로필**은 계약에 아예 없다
> (`inviter$`는 계정 `UserHead` = id + name) — 백엔드 요청이 필요하다.
