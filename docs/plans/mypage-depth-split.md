...
5. **게스트 상태**: 루트는 "로그인 필요" 카드 하나(node `4483-79911` — 나중에 받았다), 설정 depth는 게스트에게도 열되 로그아웃만 숨김. 게스트 카드는 프로필 카드와 **같은 행 구조**다(60px `DefaultAvatar` + 제목 + 부제 + chevron). 부제는 디자인이 `<복구를 위한 기능이란 문구로>` 지시문뿐이라 그 의도대로 채운 잠정 카피다.
